...
For each copy:
\begin{itemize}
    \item Constrain bits are 0 or 1. -- bits constraints for each copy, totally num\_copied*bits constraints.
    \begin{lstlisting}
for &b in &bits {
    constraints.push(builder.mul_sub_extension(b, b, b));
}
    \end{lstlisting}
    \item Constraint copy consists of bits. -- 1 constraint for each copy, totally num\_copied constraints.
    \begin{lstlisting}
...
Finally, the constant is constrained -- totally num\_extra\_constraints constraints.